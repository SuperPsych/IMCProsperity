...
PARAMS = {
    "ema_decay": 0.0001,  # EMA weight per tick. ~1/decay = effective window length
...
class Trader:
    ...
    def __init__(self) -> None:
        ...
        # EMA-based rolling mean per product, seeded from PARAMS on first tick
        self.rolling_mean: Dict[str, float] = {}

    def _rolling_mean(self, product: str, seed_key: str, mid: float) -> float:
        """Lazily seed from PARAMS[seed_key] then EMA-update with each new mid."""
        if product not in self.rolling_mean:
            self.rolling_mean[product] = float(PARAMS[seed_key])
        decay = PARAMS["ema_decay"]
        self.rolling_mean[product] = (1 - decay) * self.rolling_mean[product] + decay * mid
        return self.rolling_mean[product]
    ...
    def _trade_hydrogel(self, product: str, state: TradingState) -> List[Order]:
        ...
        mean = self._rolling_mean(product, "hydrogel_mean", (best_bid + best_ask) / 2)
    ...
    def _trade_velvetfruit(self, product: str, state: TradingState) -> List[Order]:
        ...
        limit = self.POSITION_LIMITS.get(product, 200)
        mean = self._rolling_mean(product, "velvetfruit_mean", (best_bid + best_ask) / 2)
    ...
    def _trade_voucher(self, product: str, state: TradingState) -> List[Order]:
        ...
        mean = self._rolling_mean(product, f"{product}_mean", (best_bid + best_ask) / 2)
