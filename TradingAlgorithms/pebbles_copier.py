...
class Trader:
    ...
    def __init__(self):
        self.strategies = {
            "PEBBLES_XS" : self._trade_copying,
        }
    ...
    def _trade_default(self, product, state):
        return []
    
    def _trade_copying(self, product, state):
        ...
        if market_trades and best_bid is not None and best_ask is not None:
            order = market_trades[0]
            qty = int(abs(order.quantity) * PARAMS["FRONTRUN_RATIO"])
            if order.quantity < 0:
                qty = min(qty, buy_capacity)
                if qty > 0:
                    res.append(
                        buy(product, best_bid + 1, qty)
                    )
            else:
                qty = min(qty, sell_capacity)
                if qty > 0:
                    res.append(
                        sell(product, best_ask - 1, qty)
                    )
        return res
