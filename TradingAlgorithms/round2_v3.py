...
class Trader:
    ...
    def __init__(self) -> None:
        ...
        self.last_non_spike_mid: Dict[str, float] = {}
        self.last_non_spike_spread: Dict[str, int] = {}
        # running sums for O(1) linear regression: y = a + b*x, x = step index
        self.linreg: Dict[str, Dict[str, float]] = {}
        # pepper failsafe state (reset per day by reload)
        self._pepper_cash: float = 0.0
        self._pepper_last_trade_ts: int = -1
        self._pepper_accumulated: bool = False
        self._pepper_hwm: float | None = None
        self._pepper_failsafe: bool = False

    def run(self, state: TradingState):
        if state.traderData:
            data = json.loads(state.traderData)
            self._pepper_cash = data["pepper_cash"]
            self._pepper_last_trade_ts = data["pepper_last_trade_ts"]
            self._pepper_accumulated = data["pepper_accumulated"]
            self._pepper_hwm = data["pepper_hwm"]
            self._pepper_failsafe = data["pepper_failsafe"]
        ...
        trader_data = json.dumps({
            "pepper_cash": self._pepper_cash,
            "pepper_last_trade_ts": self._pepper_last_trade_ts,
            "pepper_accumulated": self._pepper_accumulated,
            "pepper_hwm": self._pepper_hwm,
            "pepper_failsafe": self._pepper_failsafe,
        })
    ...
    def _run_pepper_strategy(self, product: str, state: TradingState) -> List[Order]:
        ...
        PARAMS = {
            ...
            "dd_threshold": 3000,   # failsafe: drawdown from post-accumulation HWM triggers hold mode
        }
        ...
        # update cumulative cash from any newly settled own trades
        max_seen_ts = self._pepper_last_trade_ts
        for trade in state.own_trades.get(product, []):
            if trade.timestamp <= self._pepper_last_trade_ts:
                continue
            if trade.buyer == "SUBMISSION":
                self._pepper_cash -= trade.price * trade.quantity
            elif trade.seller == "SUBMISSION":
                self._pepper_cash += trade.price * trade.quantity
            if trade.timestamp > max_seen_ts:
                max_seen_ts = trade.timestamp
        self._pepper_last_trade_ts = max_seen_ts

        # mark accumulation complete once position first reaches core_target
        if not self._pepper_accumulated and position >= core_target:
            self._pepper_accumulated = True

        # once accumulated, track liquidation-value HWM and check drawdown
        if self._pepper_accumulated and best_bid is not None and best_ask is not None:
            mid = (best_bid + best_ask) / 2
            liq_value = self._pepper_cash + position * mid
            if self._pepper_hwm is None or liq_value > self._pepper_hwm:
                self._pepper_hwm = liq_value
            if not self._pepper_failsafe and self._pepper_hwm - liq_value > dd_threshold:
                self._pepper_failsafe = True

        # failsafe mode: top up to limit via aggressive takes, then hold
        if self._pepper_failsafe:
            ...
            for ask_price, ask_qty in asks:
                ...
                if size > 0:
                    orders.append(buy(product, ask_price, size))
                    buy_capacity -= size
        ...
        # Use prior_fair as the fair; linreg is only a cross-check.
        # Green flag when prior and linreg agree within 2 ticks — otherwise skip
        # taking/accumulation and only penny on non-spike sides.
        linreg_fair = self._linreg_fair(product)
        green_flag = linreg_fair is None or abs(prior_fair - linreg_fair) <= 4
        fair = prior_fair
        ...
        # aggressive accumulation up to core_target — always runs, even without green flag
        aa_ceiling = fair + 8
        ...
        if green_flag:
            # spike taking: sweep all book levels within take_edge of fair
            for ask_price in sorted(order_depth.sell_orders):
                if ask_price > fair - take_edge or position >= limit:
                    break
                qty = min(limit - position, -order_depth.sell_orders[ask_price])
