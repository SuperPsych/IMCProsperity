...
class Trader:
    ...
    def _trade_osmium(self, product: str, state: TradingState) -> List[Order]:
        ...
        fair = self.last_non_spike_mid.get(product, 10_000)

        if best_bid is None or best_ask is None:
            # One side missing: penny the present side (or take it on spike),
            # and penny the empty side using last non-spike spread.
            spread = self.last_non_spike_spread.get(product)
            if best_bid is not None:
                bid_spike = best_bid > fair + spike_threshold
                sell_capacity = limit + position
                buy_capacity = limit - position
                if bid_spike:
                    if best_bid >= fair + take_edge and sell_capacity > 0:
                        qty = min(sell_capacity, best_bid_qty)
                        if qty > 0:
                            orders.append(sell(product, best_bid, qty))
                            sell_capacity -= qty
                elif buy_capacity > 0:
                    orders.append(buy(product, best_bid + 1, buy_capacity))
                if spread is not None and sell_capacity > 0:
                    assumed_ask = best_bid + spread
                    orders.append(sell(product, assumed_ask - 1, sell_capacity))
            else:
                ask_spike = best_ask < fair - spike_threshold
                buy_capacity = limit - position
                sell_capacity = limit + position
                if ask_spike:
                    if best_ask <= fair - take_edge and buy_capacity > 0:
                        qty = min(buy_capacity, -best_ask_qty)
                        if qty > 0:
                            orders.append(buy(product, best_ask, qty))
                            buy_capacity -= qty
                elif sell_capacity > 0:
                    orders.append(sell(product, best_ask - 1, sell_capacity))
                if spread is not None and buy_capacity > 0:
                    assumed_bid = best_ask - spread
                    orders.append(buy(product, assumed_bid + 1, buy_capacity))
            return orders
        ...
        if not bid_spike and not ask_spike:
            # Non-spike: update fair to current mid, penny both sides.
            self.last_non_spike_mid[product] = mid
            self.last_non_spike_spread[product] = best_ask - best_bid
            fair = mid
            ...
            sell_capacity = limit + position
            # Hit the spiked bid first if it passes fair by take_edge.
            if best_bid >= fair + take_edge and sell_capacity > 0:
                qty = min(sell_capacity, best_bid_qty)
                if qty > 0:
                    orders.append(sell(product, best_bid, qty))
                    sell_capacity -= qty
            # Penny the non-spike (ask) side with remaining capacity.
            if sell_capacity > 0:
                orders.append(sell(product, best_ask - 1, sell_capacity))
        else:  # ask_spike
            buy_capacity = limit - position
            # Lift the spiked ask first if it passes fair by take_edge.
            if best_ask <= fair - take_edge and buy_capacity > 0:
                qty = min(buy_capacity, -best_ask_qty)
                if qty > 0:
                    orders.append(buy(product, best_ask, qty))
                    buy_capacity -= qty
            # Penny the non-spike (bid) side with remaining capacity.
            if buy_capacity > 0:
                orders.append(buy(product, best_bid + 1, buy_capacity))
